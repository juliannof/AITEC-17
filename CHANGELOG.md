...
| 🟡 Media | **S2: bucle STALL→cooldown→reintento con target muy cercano al ADC actual, observado en banco (2026-08-16)** | Log real: `adc=12527 target=12321` (diferencia de solo 206 cuentas) — el motor entra en `MOVING_TO_TARGET`, se corta por `STALL — tope físico` en <500ms, espera cooldown de 2s, reintenta, se vuelve a cortar, indefinidamente (`Motor.cpp:519,859`, función `setTargetForced()`). Coincide con el patrón ya documentado como pendiente sin confirmar en la sesión 2026-07-24 20:16 (fila arriba) — no se investigó a fondo en esta sesión (foco estaba en AutoMode), solo se dejó constancia del log. Sin confirmar si el fader estaba físicamente bloqueado/sujeto en ese momento o libre. |
