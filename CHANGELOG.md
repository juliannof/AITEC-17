...
|-----------|-------|-------|
| 🟡 Media | **S2: `POSITION_FINE_PWM_K=0.5→0.2` + timeout STALL ×4 en zona fina, PENDIENTE VALIDAR LIMPIO (2026-08-20 17:39)** | Ver sesión 2026-08-20 17:39 abajo — ajuste del suelo de PWM en frenado fino + ampliación del guard STALL específicamente en `MOVING_TO_TARGET` con `absErr<POSITION_CRUISE_ERR`. Aplicado y probado en banco, pero las últimas capturas estaban contaminadas por una unidad buck defectuosa (ya sustituida) que corrompía lecturas ADC/RS485 — pendiente repetir la prueba (subir el fader hasta el tope superior desde Logic) con el hardware ya sano para saber si el fix real es suficiente. |
...
| 🟢 Baja — **RESUELTO (causa probable: cable), cerrado por el usuario (2026-08-20 18:57)** | **S3: posición 8 del bus B nunca respondía (`printStats()`), swap test confirmó que era la posición, no la unidad — causa: cable de esa posición (2026-08-20 18:48 → 18:57)** | Con el rig completo (8 slaves), `printStats()` mostró tasa de éxito 87.5% (`TX:90830 RX:79459 TIMEOUT:11652`) — el cálculo (~11.354 esperados solo del slave 8 si nunca responde) coincide casi exacto con el total de timeouts: prácticamente todo el fallo del bus viene de un único slave, los otros 7 están al ~99%. El fader de esa posición se comporta con normalidad (recibe comandos, motor actúa) — confirma que el enlace TX (S3→S2) funciona, solo la respuesta (S2→S3) nunca llega. Descartado por código: filtrado de ID en el S2 (`_rxBuf[1] != _myId`, `RS485.cpp`), dimensionado de `_ch[NUM_SLAVES+1]` (S3), ciclo de `_nextSlave()` — todo correcto, no hay bug de software encontrado. **Descartado por hardware:** el usuario sustituyó la unidad S2 física de esa posición por otra — el fallo se quedó en la posición 8, no viajó con la unidad. Apunta a cableado/conector/terminación de esa posición concreta del bus, no a un componente electrónico. Se añadió instrumentación temporal (`log_i` con `rxAvail`/`rxGot` en el timeout, `RS485.cpp`) para diferenciar "nunca llega nada" de "llega incompleto" — revertida a `log_d` tras cerrar el caso, no hizo falta usarla. **Cerrado por el usuario: causa probable, cable de esa posición.** No se tocó nada por firmware — era hardware/instalación. |
| 🔴 **CRÍTICA** | **P4: bus A RS485 con 0% de éxito — pin ENABLE (GPIO5) no conmuta el transceptor a modo escucha, HARDWARE (2026-08-20 17:52)** | `printStats()` (BRIEF_C) reveló `TX:3555 RX:0 Exito:0.0%` sostenido en 75s de captura, frente a 99.5% del S3 en el mismo protocolo. Confirmado con el usuario: GPIO3 (TX) y GPIO4 (RX) están correctos — el S2 conectado al P4 SÍ recibe comandos y se activa/calibra (confirma que TX físicamente llega), pero el P4 nunca recibe ninguna respuesta de vuelta (ni fader, ni botones, ni nada) — coherente con el transceptor quedando permanentemente en modo transmisión porque el pin de dirección (`RS485_ENABLE_PIN`=GPIO5, `config.h`) no conmuta el DE/RE correctamente. Nota histórica: la entrada anterior de este pendiente (2026-07-30) documentaba un pinout distinto (A=GPIO26, B=GPIO27, transceiver "auto-direccional sin DE/RE") que **no coincide con el `config.h` real actual** (`RS485_TX_PIN=3, RS485_RX_PIN=4, RS485_ENABLE_PIN=5`, `NUM_SLAVES=1`) — el código evolucionó sin que quedara reflejado aquí; ese pinout de 2026-07-30 queda obsoleto/sin confirmar. **No tocar por firmware** (directiva del proyecto, es hardware) — pendiente decidir si se repara la conexión física de GPIO5 al DE/RE del transceptor, o se mueve el enable a otro GPIO libre (requeriría recablear). Bloqueante para BRIEF_D (census) y cualquier trabajo real sobre el bus A. Ver [[p4_rs485_bus_a_enable_pin_roto]]. |
...
| 🟡 Media | **S2: decisión arquitectónica pendiente — recuperar sensor capacitivo (2026-08-14 19:16, actualizado)** | El heurístico de delta ADC (`setADCDelta()`) fue **eliminado por completo** el 2026-08-14 (ver sesión abajo) — ya no es "parchear vs. eliminar", está eliminado. `Motor::isManualTouchDetected()` devuelve `false` permanentemente; el gancho (declaración + consumidores) se conserva intacto para cuando el capacitivo (`FaderTouch`, desactivado desde 2026-06-14) sea fiable. La disyuntiva que queda abierta es solo esa: invertir en que el capacitivo vuelva a ser fiable, o vivir sin detección de touch indefinidamente (aceptado como diseño actual — ver `AUTOMODE.md`). Sin decisión tomada. |
| 🟢 Baja | **P4: fix de guard de familia en handshake MCU (`processMackieSysEx`), VALIDADO PARCIALMENTE (2026-08-13 16:48, actualizado 2026-08-18)** | El P4 respondía a sondeos de CUALQUIER familia Mackie (0x10/0x11/0x15/0x17), no solo la propia (0x14) — confirmado con MIDI Monitor real, bug ya documentado en `docs/MIDI.md` §3.4.1 desde 2026-05-24 pero nunca aplicado. Guard `device_family != DEVICE_FAMILY` movido al inicio de la función. **Validado en banco (2026-08-18):** captura MIDI Monitor confirma que el P4 responde correctamente a su propio serial/version reply sin degradarse. Ver sesión 2026-08-13 16:48 y sesión 2026-08-18 abajo. |
| 🟢 Baja | **S3: mismo bug de familia hardcodeada que P4 — SUPERADO por el rediseño de identidad del handshake (2026-08-13 16:48, actualizado 2026-08-18)** | Nota histórica: en 2026-08-13 se decidió deliberadamente NO tocar este guard en S3 porque conectaba bien con una sola unidad. Ese contexto quedó obsoleto tras el hallazgo de la fila 36 (serial idéntico P4/S3 rompía el reconocimiento con dos unidades) — la sesión 2026-08-18 reescribió por completo la Fase 0 del handshake en ambos MCU (serial único + version reply real), lo que resuelve también este punto de forma incidental. Ver sesión 2026-08-18 abajo. |
| 🔴 Alta | **S2+S3: rediseño del reporte de posición de fader a Logic (`SLAVE_FLAG_REPORT_FADER`), SIN VALIDAR EN BANCO (2026-08-14 19:16)** | Tras eliminar el heurístico de touch por delta ADC, se detectó que `AUTO_WRITE` (motor libre) dejaba de reportar movimientos del fader a Logic en cuanto el usuario se alejaba del `faderTarget` congelado — la heurística vieja de S3 (`motorSettled`/`FADER_SYNC_DEADBAND`) dependía de `touchState`, que ya nunca es `1`. Rediseño: cada S2 decide si reporta (usa su propio `_rsCurrentMode` + `_rsLastReportedPB`), S3 pasa a ser transparente (ya no almacena `lastSentPb[]`/heurística). Bit 6 del byte `buttons` del `SlavePacket` renombrado `SLAVE_FLAG_CALIB_SENDING` → `SLAVE_FLAG_REPORT_FADER` (el original nunca lo activaba ningún S2, confirmado antes de reutilizarlo). Ver sesión 2026-08-14 19:16 abajo. Pendiente: flashear S2+S3 juntos y validar READ (sin cambios), WRITE (fader movido a mano debe llegar a Logic en todo el recorrido, no solo cerca del punto de partida), y SELECT-por-touch (no tocado). |
| 🔴 Alta | **P4: `case 0x0E` sigue usando el SysEx de AutoMode no fiable, DELIBERADAMENTE SIN TOCAR (2026-08-16, actualizado)** | Investigación completa en S3 (ver sesión 2026-08-16 abajo) concluyó que el SysEx `0x0E` **no es una fuente fiable de AutoMode** — Logic lo manda como refresco periódico con el mismo valor para los 8 canales, no el modo real por pista — y se eliminó su uso en S3, dejando el AutoMode exclusivamente en manos de las notas 74-78 (que sí son fiables y ya tienen mapeo correcto, `noteToMode[]`). P4 nunca tuvo este problema de raíz (siempre fue Main Unit, ver hallazgo de la sesión) — su vía de notas (`MIDIProcessor.cpp:667`, `rs485.setAutoMode(g_selectedChannel + 1, newMode)`) ya reenvía correctamente. Pero P4 conserva su `case 0x0E` (líneas 575-584) sin traducir y sin reenviar por RS485 — mismo patrón obsoleto que se quitó en S3. Cuando se retome el trabajo del bus A (`NUM_SLAVES=0` en `P4/config.h`, pinout sin testear — ver pendiente arriba): eliminar el `case 0x0E` en P4 igual que se hizo en S3 (`S3/MIDIProcessor.cpp`), no portar ninguna tabla de traducción — el enfoque correcto es depender solo de notas. Ver `docs/MIDI.md` §3.4.6. |
| 🟡 Media — **PARCIALMENTE RESUELTO, VALIDACIÓN COMPLETA PENDIENTE** | **S3: `DEVICE_FAMILY=0x14` (idéntico al P4) rompía el reconocimiento MCU de Logic con P4+S3 simultáneos — revertido a 0x15 + serial único por unidad (2026-08-16 22:43 → 2026-08-18)** | Historia: el cambio a family=0x14 (2026-08-16) buscaba resolver que Logic no mandaba AutoMode (notas 74-78) al Extender, pero rompió la conexión dual P4+S3 (ambos con serial idéntico hardcodeado `00 00 00 01 00 00 00`, Logic no podía diferenciarlos). **Fix aplicado 2026-08-18 (brief de estabilidad de handshake):** `DEVICE_FAMILY`/`VERSION_REPLY_CMD` revertidos a `0x15` en S3 (`config.h:29-30`); además, cada unidad (P4 y S3) responde ahora con un **serial ASCII único** ("AITEC-M"/"AITEC-X") + challenge + version reply real de 5 bytes ("V1.00") en vez de los placeholders anteriores — ver detalle en `docs/MIDI.md` §3 Fase 0. **Validado en banco parcialmente:** captura MIDI Monitor con P4+S3 conectados simultáneamente, ~10.5s, ambas unidades estables, sin degradación a MIDI genérico, sin colisión de identidad. **Pendiente antes de cerrar:** sesión completa de >5 min con actividad de faders + reconectar proyecto de Logic (checklist completo en el brief). **Trade-off conocido y aceptado:** al volver a family=0x15, el AutoMode del Extender probablemente vuelva a no llegar por SysEx — se aborda en un brief separado (`BRIEF_automode_extender_s3link.md`), a aplicar solo después de confirmar que el handshake dual es estable. Ver sesión 2026-08-18 abajo. |
| 🟡 Media — **CAUSA RAÍZ IDENTIFICADA, FIX APLICADO SIN VALIDAR LIMPIO** | **S2: bucle STALL→cooldown→reintento con target muy cercano al ADC actual, observado en banco (2026-08-16, diagnosticado 2026-08-20 17:39)** | Log real: `adc=12527 target=12321` (diferencia de solo 206 cuentas) — el motor entra en `MOVING_TO_TARGET`, se corta por `STALL — tope físico` en <500ms, espera cooldown de 2s, reintenta, se vuelve a cortar, indefinidamente (`Motor.cpp:519,859`, función `setTargetForced()`). Causa raíz confirmada en sesión 2026-08-20 17:39 (ver abajo): el suelo de PWM en la zona de frenado fino (`_positionTick()`) caía por debajo de lo necesario para vencer fricción dinámica real, sistemáticamente cerca de donde Logic pide parar — no es atasco físico. Fix aplicado (`POSITION_FINE_PWM_K` + timeout STALL ampliado), pendiente de validación limpia tras sustituir una unidad buck defectuosa que contaminaba las últimas pruebas. |
| 🟢 Baja | **P4: UI sin indicador visual de S3 desconectado, `g_s3Connected` disponible sin consumir (2026-08-16 21:24)** | `S3Link` (P4) ya calcula `g_s3Connected` (heartbeat con el S3, ver `docs/S3LINK.md` §4) pero `UIPage3.cpp` no lo usa todavía — si el S3 se cae, las columnas 0-7 se quedan con el último dato recibido en vez de atenuarse/marcarse como offline. Pendiente: consumir la variable en el redraw de columnas 0-7. |
| 🟡 Media | **S3: `taskCore1` (botones) movido de core 1 a core 0 — fix contención con RS485, SIN VALIDAR RS485 EN BANCO (2026-08-16 22:05)** | Ver sesión 2026-08-16 22:05 abajo — el task RS485 (prioridad 5, busy-loop con `taskYIELD()`) monopolizaba el core 1 y dejaba sin CPU al task de botones (prioridad 1), percibido como botonera lenta. Confirmar tras flashear: (1) botonera responde con normalidad; (2) polling RS485 de los 8 slaves sigue estable, sin timeouts nuevos, ahora con el core 1 en exclusiva. |
| 🟡 Media | **S3: LEDs RW/FF de transporte añadidos a `setLedByNote()` (case 97→92, nuevo case 91), SIN VALIDAR EN BANCO (2026-08-16 22:10)** | Ver sesión 2026-08-16 22:05 abajo, Hallazgo 4 — antes nunca recibían feedback real de Logic (nota FF errónea, nota RW ausente). Confirmar en Logic que ambos LEDs responden al rebobinar/avanzar. |
| 🟡 Media | **S3: LED STOP encendido localmente al pulsar el botón físico, SIN VALIDAR EN BANCO (2026-08-16 22:30)** | Ver sesión 2026-08-16 22:05 abajo, Hallazgo 5 (cerrado con captura MIDI Monitor real) — confirmado que Logic nunca manda feedback de la nota 93 al pulsar Stop, así que `onButtonPressed()` enciende `LED_STOP` localmente sin esperar a Logic. Confirmar en banco: STOP se enciende al pulsarlo y se apaga correctamente con PLAY/FF/RW. |
...
### SESIÓN 2026-08-20 17:39 — S2/S3: instrumentación RS485+display, deadband PitchBend, frenado fino del motor, y hallazgo de hardware (unidad buck defectuosa)

**Origen:** petición inicial de conexión ESP-NOW S3→C3 para encender un rail de 10V tras un census de S2 vivos (plan maestro BRIEF A-G). El análisis previo señaló que la latencia variable de respuesta del S2 en el bus RS485 era el problema de fondo real, y esta sesión se dedicó por completo a medir e investigar esa cadena — el plan ESP-NOW/census (BRIEF D/E/F/G) quedó sin tocar, solo BRIEF_A se aplicó.

**MCU afectadas:** S2 ✅ · S3 ✅ (deadband) · P4 ❌.

**BRIEF_A aplicado — instrumentación `micros()` en el S2 (`main.cpp`):** máximos por bloque cada 5s (`[INSTR] MAX(us) onMaster=.. build=.. send=.. rx2send=.. motor=.. disp=.. neo=..`). Reveló que el bloque RS485 en sí es rápido (~500µs en reposo), muy por debajo de los timeouts del master — la hipótesis original de BRIEF_B (invertir orden `onMasterData`/`buildResponse`/`sendResponse`) no ataca la causa dominante real. **BRIEF_B NO se aplicó** — quedó descartado por los datos.

**Hallazgo 1 — redraw total del display bloqueaba el loop hasta 247ms:** instrumentado `updateDisplay()`/`drawSplashScreen()` en `Display.cpp` (`[INSTR-REDRAW]`, `[INSTR-SPLASH]`). El bloqueo de 247ms ocurre en la cola de render (fuera del bloque RS485), disparado por `needsTOTALRedraw` en cambios de conexión — durante ese tiempo el S2 no atiende el bus en absoluto. Desglose: `fill` (fillScreen) ~110ms dominaba, seguido de `main` (drawMainArea) ~79ms. Cálculo teórico confirmó que el SPI del display corría a solo 10MHz (`LovyanGFX_config.h`) — 134.400 bytes/10Mbit/s ≈ 107ms, coincidente con lo medido.

**Fix 1 — SPI del display 10MHz→80MHz** (`LovyanGFX_config.h`): `fillScreen()` bajó de ~110ms a ~14ms (factor ~7.8x, validado con datos reales). Redraw total 243.7ms→43.1ms.

**Hallazgo 2 — vibración/STALL del motor, reproducible en cualquier S2 conectado al S3:** el usuario confirmó que el problema no era de una unidad física concreta sino del master S3. Encontrado: `PITCHBEND_DEADBAND` divergente entre masters — S3=80, P4=150 (ya anotado como pendiente técnico previo). Con margen más estrecho, el S3 dejaba pasar más jitter del PitchBend de Logic hacia el bus, causando actualizaciones de target más frecuentes al S2.

**Fix 2 — `PITCHBEND_DEADBAND` 80→150 en S3** (`MASTER_S3-P4/S3/.../MIDIProcessor.cpp`), alineado con el valor ya validado en P4.

**Hallazgo 3 — falso STALL sistemático "donde para Logic":** el usuario confirmó que el STALL siempre ocurre justo al llegar al target, en cualquier punto del recorrido (no un punto fijo del carril). Causa: `_positionTick()` (`Motor.cpp`) reduce el PWM cuadráticamente cerca del target (`POSITION_FINE_PWM_K`), cayendo por debajo del PWM de arranque ya calibrado — insuficiente para vencer fricción dinámica real, especialmente cerca del tope superior. La calibración (PWM alto y sostenido) sí completa el recorrido; el frenado normal, no.

**Fix 3 — `POSITION_FINE_PWM_K` 0.5→0.2** (`config.h`) — sube el suelo de PWM en la zona de frenado fino.
**Fix 4 — timeout STALL ×4 en zona fina** (`Motor.cpp`, `_inFineBraking`) — en `MOVING_TO_TARGET` con `absErr<POSITION_CRUISE_ERR`, el guard `STALL_PROTECT` usa 1600ms en vez de 400ms antes de cortar, en vez de eliminar la protección por completo (mantiene la red de seguridad ante un atasco físico real).

**Hallazgo 4 — CAUSA RAÍZ REAL de la corrupción de datos: unidad buck defectuosa, no EMI del SPI.** Tras Fix 1 (SPI 80MHz), aparecieron saltos de posición del fader físicamente imposibles (miles de counts de ADC en <300ms, muy por encima de la velocidad máxima real del motor), tanto en lecturas ADC locales (I2C) como en `faderTarget` recibido por RS485 — ambos buses afectados a la vez, lo que ya apuntaba a algo más genérico que EMI de un bus concreto. Se bajó el SPI a 40MHz como diagnóstico intermedio y se preparó un protocolo de medición con osciloscopio (SCLK del display vs SDA/SCL del ADS1115), con aviso explícito de no confundir el resultado con el ground loop ya conocido de esta ATX (rails 10V/5V separados — mismo origen que `s3_led_rec_fantasma_hardware`, S3LINK). **El usuario identificó y sustituyó la unidad buck defectuosa antes de completar la medición con osciloscopio — el problema desapareció.** Topología confirmada: rail 10V exclusivo para el motor, rail 5V para toda la lógica (ESP32, display, ADS1115, RS485). Con el hardware sano, el SPI se devolvió a 80MHz (Fix 1 restaurado).

**Riesgo del hallazgo 4:** este buck defectuoso probablemente contribuía también al patrón de vibración/STALL original que motivó toda la investigación del motor — no se puede separar limpiamente qué parte del comportamiento errático era el algoritmo de frenado (Fix 3/4) y qué parte era la alimentación inestable. **Pendiente:** repetir la prueba de subir el fader hasta el tope superior desde Logic con el hardware ya sano, para confirmar si Fix 3/4 son suficientes o si el comportamiento ya era correcto y el buck explicaba todo.

**No se tocó:** `STALL_PROTECT_MS` base, el umbral de 10 counts, ni ningún timeout de RS485 — por directiva del proyecto (prohibido enmascarar problemas físicos con firmware). BRIEF_C, D, E, F, G del plan maestro original no se ejecutaron en esta sesión.
...
### SESIÓN 2026-08-18 15:37 — P4+S3: serial único en handshake MCU (estabilidad dos unidades) | P4: fix lag de timecode en vista VU meters

**Origen:** brief del usuario "BRIEF_automode_extender_s3link.md" (parte 1, estabilidad — el AutoMode del Extender va en un brief separado, después). Diagnóstico previo del usuario, verificado con captura MIDI real de un BCF2000 en modo Mackie: con P4+S3 conectados simultáneamente, Logic funciona un rato y luego degrada ambas superficies a MIDI genérico porque ambas responden al sondeo `0x00` con el mismo serial hardcodeado.

**MCU afectadas:** P4 ✅ · S3 ✅ · S2 ❌ (no participa del handshake Mackie).

---

**Cambio 1 — Serial único + challenge + version reply real (`processMackieSysEx`, ambos MCU):**

Antes, `command == 0x00` respondía con un serial placeholder idéntico (`00 00 00 01 00 00 00`) en P4 y S3, y `command == 0x13` respondía con solo 2 bytes en vez de una versión ASCII real. Fix:
- P4 (`P4_JC1060P470C/src/midi/MIDIProcessor.cpp`): serial "AITEC-M" + challenge "AITM"; version reply "V1.00" (5 bytes ASCII).
- S3 (`S3/iMakie-ESP32_S3_EXTENDER/src/midi/MIDIProcessor.cpp`): serial "AITEC-X" + challenge "AITX"; version reply "V1.00".
- Ambos: nueva rama `command == 0x02` → responde `0x03` con el mismo serial, sin validar el challenge (patrón "B1: confianza", igual que la BCF2000 real). **Observado en banco:** Logic nunca llegó a enviar el `0x02` en las capturas de esta sesión — la rama queda como salvaguarda inerte para este caso, el fix real es el serial único.

**Cambio 2 — S3: `DEVICE_FAMILY`/`VERSION_REPLY_CMD` revertidos `0x14`→`0x15` (`config.h:29-30`):**

El experimento de 2026-08-16 (identificarse como Main Unit para intentar recibir AutoMode) quedó revertido — coincidía exactamente con el bug de serial idéntico de arriba. Se documenta el trade-off conocido: con `0x15`, el AutoMode del Extender probablemente vuelva a fallar (a resolver en el brief separado de AutoMode).

**Validación en banco (parcial):**
- Captura MIDI Monitor con P4 solo: confirmado serial "AITEC-M"+challenge y version reply "V1.00" en tráfico real.
- Captura MIDI Monitor con **P4+S3 simultáneos** (~10.5s): ambas unidades mantienen `CONNECTED` estable, reciben datos de fader/nombre/VU/AutoMode correctamente direccionados por familia sin colisión, sin degradar a MIDI genérico en la ventana observada.
- **Pendiente:** sesión de >5 min con actividad de faders, y prueba de reconexión (cerrar/reabrir proyecto de Logic) — ver tabla de pendientes arriba.

**Hallazgo colateral — no es un bug de firmware:** durante las capturas apareció tráfico SysEx `0x12` con texto de diálogo de Logic ("¿Seguro que quieres eliminar todos los dispositivos sel...? Cancelar/Eliminar") y Note On/CC sin relación con controles reales. Confirmado: es la ventana **Control Surfaces → Setup** de Logic filtrando el texto de sus propios diálogos por el mismo canal SysEx que usa para nombres de pista — el firmware pinta fielmente lo que recibe, no hay nada que corregir. Documentado en `docs/MIDI.md` §4.2 para no confundirlo con un bug real en próximas sesiones. También explica las ráfagas de reconexión (`0x21→0x20×8→...`) vistas mientras esa ventana estaba en uso.

---

**Cambio 3 — P4: invalidación selectiva de VU meters (fix lag de timecode):**

Reportado por el usuario: en la vista de VU meters del P4, en cuanto empiezan a dibujarse (playback activo), el timecode de la cabecera tarda en actualizar. Causa raíz (leída directamente en código, sin necesidad de banco): `uiPage3Update()` invalidaba las 16 columnas VU completas en cada tick de `needsVUMetersRedraw`, aunque solo hubiera cambiado 1 canal — con el buffer LVGL de 100 líneas (`VU_H≈296px`), eso forzaba varias pasadas de flush bloqueantes por ciclo dentro del mismo bucle de `taskCore1` que actualiza el timecode, retrasándolo.

**Fix:** nuevo array `vuDirty[16]` — cada punto que marca `needsVUMetersRedraw=true` (`MIDIProcessor.cpp`, `S3Link.cpp`, `UIPage3.cpp::handleVUMeterDecay()`) ahora también marca `vuDirty[canal]=true` solo para el canal que cambió; `uiPage3Update()` invalida únicamente esas columnas. Detalle completo en `docs/BUG_VU_P4.md` §"BUG 2". **Sin validar en banco todavía.**

---

**Archivos modificados esta sesión:**
- `MASTER_S3-P4/P4_JC1060P470C/src/midi/MIDIProcessor.cpp` — serial, challenge, version reply, `vuDirty`
- `MASTER_S3-P4/S3/iMakie-ESP32_S3_EXTENDER/src/config.h` — `DEVICE_FAMILY`/`VERSION_REPLY_CMD` → 0x15
- `MASTER_S3-P4/S3/iMakie-ESP32_S3_EXTENDER/src/midi/MIDIProcessor.cpp` — serial, challenge, version reply
- `MASTER_S3-P4/P4_JC1060P470C/src/config.h`, `src/main.cpp`, `src/S3Link/S3Link.cpp`, `src/display/UIPage3.cpp` — `vuDirty[16]`
- `docs/MIDI.md` §3 (Fase 0), §4.2 · `docs/BUG_VU_P4.md` (nueva sección)

**Riesgo:** MEDIO (handshake MCU, requiere validación completa en banco antes de dar por cerrado) + BAJO (fix de display, aislado de RS485/Motor/protocolo).

---

### SESIÓN 2026-08-16 22:05 — S3: brillo fantasma en LED REC (GPIO flotante en boot) + botonera lenta (contención de core FreeRTOS)

**Origen:** el usuario reportó un brillo tenue "fantasma" en el LED REC del transporte al arrancar/conectar el S3, y por separado, que la botonera se percibía lenta ("no podemos permitirnos esto").

**MCU afectadas:** S3 ✅ (únicas afectadas, ambos hallazgos) · S2 ❌ · P4 ❌.

---

**Hallazgo 1 — LED REC fantasma (`Transporte.cpp`, `main.cpp`, `config.h`):**

Diagnóstico por descarte con el usuario (brillo "muy tenue", solo en boot, solo en REC): fuga de corriente por GPIO12 (`LED_REC`) en alta impedancia durante la ventana entre power-on y el primer `pinMode(OUTPUT)` del firmware — corriente de fuga vía el diodo ESD interno del pin, del orden de microamperios frente a los ~mA de un LED a plena corriente (consistente con "solo visible en penumbra"). Se descartaron diafonía con `BTN_REC` (GPIO11) y rizado de tierra por USB, ya que ninguna encaja con un brillo tan débil y sostenido.

**Fix 1:** nueva función `Transporte::initPins()` (`Transporte.cpp`) — solo `pinMode(OUTPUT)`+apagado de los 5 LEDs de transporte, sin el test visual de `begin()`. Llamada como **primerísima instrucción de `setup()`** (`main.cpp:287-289`), antes de `randomSeed()`/`Serial.begin()`/`USB.begin()`, para minimizar la ventana software en la que los 5 GPIO quedan flotantes. `Transporte::begin()` se mantiene sin cambios en su sitio original (repite el `pinMode`/apagado, redundante pero inofensivo, y conserva el test visual de arranque).

**Validado en banco:** confirmado por el usuario — el fantasma desapareció tras aplicar `initPins()` combinado con el brillo PWM reducido (ver Hallazgo 2). No se puede afirmar con certeza cuál de los dos cambios fue determinante por sí solo.

---

**Hallazgo 2 — brillo PWM ajustable en los 5 LEDs de transporte (`config.h`, `Transporte.cpp`):**

Petición del usuario, independiente del fantasma: pasar los 5 LEDs de transporte (REC/PLAY/FF/STOP/RW) de on/off binario (`digitalWrite`) a brillo ajustable. El S3 no tenía uso previo de LEDC (NeoPixel usa RMT, sin display/backlight en este proyecto) — sin riesgo de conflicto de canales.

**Fix:** nueva constante `TRANSPORT_LED_BRIGHTNESS` (`config.h:109-110`, fuente única de verdad). `Transporte::setLed()` pasa de `digitalWrite(pin, on?LOW:HIGH)` a `analogWrite(pin, on ? (255-TRANSPORT_LED_BRIGHTNESS) : 255)` — invertido porque la lógica es ánodo común/sink (`LOW`=más brillo) y `analogWrite()` interpreta su valor como % de tiempo en `HIGH`. Valor final tras pruebas en banco: **`TRANSPORT_LED_BRIGHTNESS = 50`** (0-255).

---

**Hallazgo 3 — botonera lenta: contención de CPU entre tasks del mismo core (`main.cpp`):**

Investigado a raíz del reporte del usuario. Causa real: `taskCore1` (botones de transporte, `Transporte::update()` → `Button2::loop()`, prioridad 1) y el task RS485 (`RS485Master::runTask()`, prioridad 5) estaban **pineados al mismo core (1)**. El task RS485 es un bucle continuo que nunca se bloquea de verdad — solo hace `taskYIELD()` entre estados (`RS485.cpp:143`), que solo cede CPU a tareas de igual o mayor prioridad. Al estar casi siempre listo (máquina de estados de polling con timings en microsegundos) y con prioridad muy superior, monopolizaba el core 1, dejando al task de botones sin CPU real — percibido como lentitud en la botonera. No relacionado con los cambios de LED de esta misma sesión; arquitectura de tasks preexistente.

**Fix (`main.cpp:329-332`):** `taskCore1` movido del core 1 al **core 0** (que ya aloja `taskCore0`/MIDI, prioridad 2, con `vTaskDelay(1)` real cada vuelta). El core 1 queda dedicado en exclusiva al RS485, que es el que necesita timing de microsegundos. No se tocó `RS485.cpp` ni ningún timing/protocolo del bus.

**PENDIENTE VALIDAR EN BANCO:** confirmar que el polling RS485 de los 8 slaves sigue estable (sin timeouts nuevos) tras liberar el core 1 en exclusiva, y que la botonera responde con normalidad.

---

**Hallazgo 4 — LEDs RW y FF nunca recibían feedback de Logic (`Transporte.cpp::setLedByNote()`):**

A raíz de una pregunta del usuario sobre si el protocolo Mackie ilumina RW/FF (sí — notas 91/92, mismo mecanismo que PLAY/STOP/REC, ya documentado en `docs/MIDI.md` líneas 945-953), se confirmó que el `switch` de `setLedByNote()` no cubría esas notas: `case 95` decía en el comentario "REWIND" pero en realidad maneja RECORD (95=0x5F, funcionalmente correcto, solo el comentario estaba mal); `case 97` decía "FORWARD" pero 97 no es una nota MCU real de transporte — la nota FF real es 92 (0x5C); no existía ningún `case 91` (RW). Resultado: aunque Logic mande el feedback correcto, los LEDs RW y FF nunca se encendían.

**Fix:** `case 97` → `case 92` (FF real), nuevo `case 91` (RW) añadido con el mismo patrón que REC/PLAY, comentario de `case 95` corregido.

**PENDIENTE VALIDAR EN BANCO:** confirmar en Logic que los LEDs RW/FF ahora responden al rebobinar/avanzar.

---

**Hallazgo 5 — LED STOP no se apagaba con FF/RW (`Transporte.cpp::setLedByNote()`) — dos iteraciones:**

Reportado por el usuario en banco tras el fix del Hallazgo 4: al usar FF/RW, el LED STOP se quedaba encendido. Causa: `case 94` (PLAY) inferí a STOP como su inverso (`setLed(LED_STOP, !on)`) — combo heredado del código original. Nada relacionado con FF/RW podía apagar STOP.

**Intento 1 (22:15, revertido):** se separó STOP en `case 93` propio, asumiendo que Logic lo manda de forma explícita y fiable. **Falso** — confirmado en banco por el usuario: "stop no se apaga", el LED dejó de apagarse en absoluto (ni siquiera en el ciclo normal play↔stop, que sí funcionaba antes de tocar nada).

**Intento 2 (22:18, también revertido):** restaurado el combo PLAY/STOP (`case 94`), conservando `case 93` como vía secundaria "por si acaso". Resultado reportado por el usuario: STOP dejaba de **encenderse** al pulsarlo (FF/RW sí funcionaban bien). Sospecha: Logic manda un Note Off de la nota 93 como flash momentáneo del botón físico, que pisa el `true` que el combo ya había puesto correctamente para STOP.

**Fix definitivo (22:25):** `case 93` eliminado por completo — dos intentos con esa nota causaron dos regresiones distintas, sin datos de MIDI Monitor que confirmen cómo la usa Logic realmente. STOP depende exclusivamente de: combo con PLAY (`case 94`, la única vía validada desde el origen) + apagado explícito al activar FF/RW (`case 91`/`92`, aditivo, no toca el combo).

**PENDIENTE VALIDAR EN BANCO:** (1) confirmar que STOP se enciende correctamente al pulsarlo; (2) confirmar que STOP se apaga correctamente en el ciclo normal play↔stop; (3) confirmar que FF/RW apagan STOP al activarse. **Si algo de esto sigue fallando, el siguiente paso es una captura de MIDI Monitor de la nota 93 (0x5D) — no seguir iterando el código a ciegas** (ver `docs/Transport.md` §2.2).

---

**Cierre del Hallazgo 5 — captura MIDI Monitor real (2026-08-16 22:28-22:30):**

El usuario aportó una captura real de MIDI Monitor con pulsaciones físicas de RW/FF/STOP. Decodificando la nomenclatura de notas (`(octava+2)×12+clase_de_tono`): 91=G5(RW), 92=G♯5(FF), 93=A5(STOP), 94=A♯5(PLAY). Siguiendo la secuencia `From iMakie-Extender` (pulsación real) → `To iMakie-Extender` (respuesta de Logic):

```
From Note On G5 (RW) → To: Note Off G♯5 (Logic apaga FF)         — sin mención de A5/STOP
From Note On A5 (STOP) → To: Note Off G5 (Logic apaga RW)        — sin mención de A5/STOP
```

**Confirmado con datos reales:** Logic responde con eco fiable a FF y RW (sus propias notas), pero **nunca** manda ningún mensaje para la nota 93 (STOP) como confirmación de la pulsación física — en toda la captura, `A5` como mensaje `To` (Logic→S3) aparece una única vez, dentro de la ráfaga de reset de conexión completo (ver `MIDI.md` §3.3, "Note Off masivo"), no como respuesta a un botón. Esto confirma definitivamente que la eliminación de `case 93` fue correcta, y explica el bug pendiente: FF/RW apagan STOP por código propio, pero nada lo volvía a encender, porque Logic no da ninguna señal fiable de "transporte parado" al pulsar Stop directamente.

**Fix final — `Transporte.cpp::onButtonPressed()`:** `LED_STOP` se enciende **localmente**, de inmediato, al detectar la pulsación física del botón STOP (`if (LEDS[i] == LED_STOP) setLed(LED_STOP, true);`), sin esperar a Logic — la única fuente fiable disponible, según los datos. REC/PLAY/FF/RW se mantienen gobernados por feedback real de Logic (confirmado fiable para esas 4).

**PENDIENTE VALIDAR EN BANCO (definitivo):** confirmar en banco que STOP ahora se enciende al pulsarlo y se apaga correctamente con PLAY/FF/RW.

---

### SESIÓN 2026-08-16 20:06 — Nuevo enlace serie S3↔P4 (Serial2): datos de canal + heartbeat

**Origen:** hasta ahora P4 y S3 eran dos masters MCU/RS485 completamente aislados (cada uno con su propio USB-MIDI y su propio bus de slaves S2) — la pantalla del P4 tenía columnas reservadas para los 8 canales del S3 (`P4_CH_OFFSET=8`, ya presente en `config.h`) pero nunca alimentadas. El usuario pidió que el S3 reenvíe al P4 nombre de pista, REC/MUTE/SOLO/SELECT y vúmetros de sus 8 canales, más un heartbeat en sentido P4→S3 para que el P4 sepa si el S3 sigue vivo.

**MCU afectadas:** S3 ✅ · P4 ✅ · S2 ❌ (sin cambios, el nuevo enlace es ajeno a los buses RS485 existentes).

**Diseño:** UART físico nuevo (`Serial2`) en ambos MCU, independiente del `Serial1` que cada uno ya usa para su propio RS485. Protocolo binario nuevo (`s3_link_protocol.h`, duplicado por proyecto igual que `protocol.h`) con CRC8 del mismo polinomio: `S3LinkChannelFrame` (13B, S3→P4, reutiliza el formato ya validado `trackName[7]`+flags+vuLevel de `MasterPacket`) y `S3LinkPingPongFrame` (3B, PING P4→S3 / PONG S3→P4). Módulo nuevo `S3Link` (una copia por MCU) enganchado en los mismos puntos donde el S3 ya alimenta su RS485 propio (`midi/MIDIProcessor.cpp`), sin duplicar parseo MIDI. Detalle completo: `docs/S3LINK.md`.

**Pines** (dos candidatos descartados antes de cerrar el diseño — ver `docs/S3LINK.md` §2): S3 GPIO37 (reservado por PSRAM Octal del módulo, `qio_opi`) y S3 GPIO1 (ya usado por `RS485_ENABLE_PIN` del bus propio). Pines finales: P4 TX=GPIO1/RX=GPIO2, S3 TX=GPIO18/RX=GPIO17, 115200 baud, cableado cruzado + GND común.

**Validado en banco:** boot OK en ambos lados; heartbeat sin pérdidas tras ~4.7 min (`ping=563 pong=563`); flujo de datos de canal (nombre/REC/MUTE/SOLO/SELECT/VU) confirmado visible en las columnas 0-7 de la pantalla del P4 tras mover fader/pulsar botones en el S3.

**Pendiente:** `g_s3Connected` (heartbeat) no se consume todavía en la UI del P4 — ver tabla de pendientes arriba. Log de diagnóstico del heartbeat (`[S3LINK] connected=... ping=... pong=...` cada 2s en el P4) dejado activo a propósito para seguir observando en banco.

---

### SESIÓN 2026-08-16 — S3: AutoMode no se reflejaba en el S2 — causa raíz real: Extender nunca recibe notas 74-78

**Origen:** el usuario reportó, con log de MIDI Monitor real, que el modo de automatización de pista se refleja correctamente en P4 pero no en el S2 (vía S3). La investigación tuvo dos fases: un primer fix que resultó insuficiente, y el hallazgo de la causa raíz real tras seguir investigando con más pruebas en banco.

**MCU afectadas:** S3 ✅ (todos los cambios) · P4 ❌ (mismo patrón de código obsoleto, deliberadamente sin tocar — ver tabla de pendientes) · S2 ❌ (su lógica de recepción siempre fue correcta).

---

**Fase 1 — fix de mapeo de valores (parcialmente correcto, luego revertido):**

Descartado el transporte completo paso a paso (Logic sí manda `0x0E` a S3 — confirmado en log; S3 sí reenviaba por RS485; el S2 sí aplicaba bien lo que recibía), la causa parecía estar en el valor: el protocolo Mackie codifica el AutoMode por dos vías con dos órdenes numéricos distintos — notas 74-78 (`noteToMode[5]`, orden correcto y ya usado por el enum interno `AutoMode`) vs. SysEx `0x0E` (documentado en `docs/MIDI.md` §3.4.6 con un orden distinto: `0=Off 1=Read 2=Write 3=Touch 4=Latch 5=Trim`). Se aplicó una tabla de traducción `mackieToInternal[6]` en el `case 0x0E` de S3. **Este fix quedó revertido en la Fase 2** — no por estar mal construido, sino porque se descubrió que la fuente (`0x0E`) no es fiable en absoluto, así que traducirla bien no arregla el problema real.

---

**Fase 2 — causa raíz real: Logic nunca manda notas de AutoMode a un Extender**

Con logs de diagnóstico añadidos (`log_i` en el `case 0x0E` de S3 y en `RS485Handler.cpp` del S2, más un resumen periódico `[AUTOMODE-ALL]` en `S3/main.cpp`), se observó que el SysEx `0x0E` llega como un **refresco periódico con el mismo valor para los 8 canales de golpe** — no refleja el modo real e individual de cada pista. Mientras tanto, cambiando el AutoMode de una pista con los botones físicos (notas 74-78), se confirmó en MIDI Monitor que Logic **solo manda esas notas al P4** ("To iMakie-P4-Master"), nunca a "To iMakie-Extender" — ni siquiera seleccionando esa misma pista directamente en el banco del S3 (se probó explícitamente y se descartó que fuera un tema de selección).

Esto es un comportamiento **estructural del protocolo Mackie Control**: el AutoMode por nota es una función del **Main Unit**, no de los **Extenders** — un Extender es, en la spec, un banco adicional de 8 canales sin esa función. Confirmado indirectamente por P4: su tabla de colores en `UIPage3B.cpp` indexa el byte Mackie crudo con un orden que, validado en banco por el usuario con múltiples pistas en modos reales distintos, resultó correcto — el P4 nunca tuvo este problema porque siempre fue tratado como Main Unit.

**Fix — `S3/config.h:12-19`:** `DEVICE_FAMILY` y `VERSION_REPLY_CMD` cambiados de `0x15` (Extender) a `0x14` (idéntico al P4, Main Unit). Tras el cambio, confirmado en banco con MIDI Monitor: las notas 74-78 empezaron a llegar también "To iMakie-Extender". **Riesgo conocido:** este mismo valor causó antes, por error de copia, que "Logic no pudiera distinguir master de extender" (fix `2026-08-07`, comentario en el propio archivo) — no se conoce el síntoma exacto de aquella vez. Queda pendiente confirmar que el banking de canales sigue sincronizado entre P4 y S3, y que la conexión general se mantiene estable a largo plazo (ver tabla de pendientes). Reversible: volver `0x14`→`0x15` si aparece cualquier síntoma.

**Fix — `S3/MIDIProcessor.cpp` `case 0x0E`:** eliminado el procesamiento de AutoMode por este SysEx (incluida la tabla `mackieToInternal` de la Fase 1) — con las notas ya llegando de forma fiable, dejar el `0x0E` activo solo arriesgaba pisar con su valor placeholder el modo correcto que acaba de llegar por nota. El AutoMode en S3 pasa a depender exclusivamente de las notas 74-78.

**Fix — `S3/MIDIProcessor.cpp` `case 0x0F` (GoOffline):** al desconectar Logic, los 8 canales se resetean a `AUTO_READ` (`g_channelAutoMode[]` + `rs485.setAutoMode()` a cada S2) — antes se quedaban con el último modo real recibido, mostrando un estado obsoleto mientras Logic estaba desconectado. Combinado con el default de arranque (`g_channelAutoMode[8]` ya inicializado a `AUTO_READ`, línea 71-73, decisión previa de 2026-07-30), el modo queda en READ tanto al arrancar como al desconectar, y solo cambia cuando Logic manda el modo real por nota.

**P4 — deliberadamente SIN TOCAR (decisión del usuario, 2026-08-16):** conserva su `case 0x0E` sin traducir y sin reenviar por RS485 (patrón ya obsoleto, igual al que se quitó en S3) — no se toca hasta retomar el trabajo del bus RS485 A. Cuando se retome, el enfoque correcto es **eliminar** ese `case 0x0E` (como en S3), no portarle una tabla de traducción — P4 ya recibe el AutoMode real y fiable por notas.

**PENDIENTE VALIDAR EN BANCO:** (1) banking sincronizado P4↔S3 tras el cambio de `DEVICE_FAMILY`; (2) estabilidad de conexión del S3 a largo plazo; (3) que el reset a READ en desconexión no interfiera con una reconexión rápida.

---

### SESIÓN 2026-08-13 16:48 — P4: fix guard de familia en handshake MCU (loop de reconexión)

**Origen:** continuación de la sesión de hoy. Tras el commit de S2/S3 (16:13/16:16), el usuario reportó en banco: color de header no refleja AutoMode, botón SELECT no funciona, luego "todos los botones han dejado de funcionar" en el S2. Investigación de código (ButtonManager.cpp, Hardware.cpp, RS485Handler.cpp) no encontró ningún cambio de esta sesión que explicara el síntoma. El usuario aportó el dato clave: "tenemos un problema al incluir P4, seguramente el handshake es incorrecto y hace que Logic pase de las superficies de control."

**MCU afectadas:** P4 ✅ (único cambio) · S3 ❌ (mismo bug presente, deliberadamente no tocado — confirmado OK en Logic) · S2 ❌.

**Diagnóstico (MIDI Monitor, captura 2026-08-13 16:37:46):** Logic sondea 5 familias Mackie (`0x10`, `0x11`, `0x17`, `0x14`, `0x15`) preguntando "¿eres tú?" a cada una. El P4 respondía a **todas** con su propia identidad (`0x14`), sin comprobar `device_family` antes de contestar a los comandos `0x00`/`0x13`. La captura muestra la secuencia de conexión completa (`0x21` GoOnline → `0x20`×8 Fader Touch Sensitivity → `0x0A` → `0x0C` → `0x0B` → `0x12` LCD clear) repitiéndose cada ~200-300ms de forma indefinida (16:37:47.088, .158, .338, .571, .785, 48.002...) sin llegar nunca a un estado estable — Logic reinicia la negociación una y otra vez al recibir respuestas inesperadas a sus sondeos de otras familias.

Este bug ya estaba documentado en `docs/MIDI.md` §3.4.1 desde el 2026-05-24 ("Bug crítico — P4 y S3 responden a CUALQUIER familia"), con el fix correcto ya identificado entonces ("mover el guard al inicio"), pero nunca se aplicó. El commit `a0fb43b` (2026-08-07) corrigió un bug relacionado pero distinto en S3 (identidad equivocada — el S3 respondía `0x14`, la identidad del P4, en vez de `0x15` la propia) sin mover el guard de sitio — el S3 sigue con el mismo patrón estructural, pero el usuario confirmó que conecta bien en Logic tal como está, así que se deja intacto.

**Fix — P4 `src/midi/MIDIProcessor.cpp::processMackieSysEx()`:** guard `if (device_family != DEVICE_FAMILY) return;` movido de después de los bloques `command==0x00`/`0x13` a justo después de leer `device_family`/`command`, antes de cualquier respuesta. Los dos `0x14` hardcodeados en las respuestas de `0x00` y `0x13` sustituidos por `DEVICE_FAMILY` (mismo valor, pero consistente con `config.h` como fuente única de verdad). Guard duplicado que quedaba más abajo, eliminado.

**SIN VALIDAR EN BANCO** — pendiente reflashear P4 y confirmar con MIDI Monitor que solo responde al probe de familia `0x14` y que la secuencia de conexión se estabiliza en un solo intento.

---

### SESIÓN 2026-08-14 19:16 — S2: eliminación completa de `setADCDelta()` + rediseño del reporte de fader a Logic (`SLAVE_FLAG_REPORT_FADER`)

**Origen:** ejecución de la decisión que quedó pendiente en la sesión 2026-08-13 15:10 (heurístico de touch por delta ADC vs. capacitivo) — el usuario confirmó eliminar el heurístico por completo, no seguir parcheándolo. Al eliminarlo se detectó una regresión: `AUTO_WRITE` dejaba de reportar el movimiento manual del fader a Logic.

**MCU afectadas:** S2 ✅ (`RS485Handler.cpp`, `config.h`, `Motor.cpp`, `Motor.h`, `main.cpp`, `protocol.h`) · S3 ✅ (`config.h`, `main.cpp`, `protocol.h`) · P4 ❌ (bus A sin activar, no aplica).

**1. S2 — `hardware/Motor/Motor.cpp`/`.h` + `main.cpp` + `config.h` — eliminada `Motor::setADCDelta()`**
- Función completa (antes ~L646), su declaración en `Motor.h`, su única llamada en `main.cpp::loop()`, y las constantes que solo ella consumía en `config.h` (`MANUAL_TOUCH_THRESHOLD`, `MANUAL_TOUCH_AT_TARGET_THRESHOLD`, `TOUCH_DELTA_WINDOW_MS`, `AT_TARGET_TOUCH_GRACE_MS`, `_deltaWindowStart`, `_deltaWindowRef`) — todo eliminado.
- `Motor::isManualTouchDetected()` se conserva como gancho (declaración + consumidores en `RS485Handler.cpp`/`goToMin()`/`setTargetFromS3()`) pero devuelve `false` permanentemente — reservado para cuando `FaderTouch` (capacitivo, desactivado desde 2026-06-14) vuelva a ser fiable.
- `MANUAL_TOUCH_DEBOUNCE_MS` (600ms) se conserva porque `RS485Handler.cpp::_touchDebounceForMode()` (rama `default`) la reutiliza como debounce genérico de `touchState` — rama hoy inalcanzable en la práctica (nunca hay touch real que la dispare), pero sigue siendo código vivo.
- **Consecuencia de seguridad aceptada** (documentada en el propio comentario de código): con el fader parado, sujetarlo con la mano ya NO cede el control al motor — el motor perseguirá el target de Logic contra la mano, cortándose por `STALL_PROTECT_MS` (~400ms, mecanismo independiente, intacto) y reintentando tras `STALL_COOLDOWN_MS`.

**2. S2 — `RS485/RS485Handler.cpp::buildResponse()` + `config.h` — nueva lógica de reporte de fader por AutoMode**
- Cada S2 decide ahora si reporta `faderPos` a Logic según su propio `_rsCurrentMode`:
  - `AUTO_WRITE`: reporta si `|pb - _rsLastReportedPB| > FADER_REPORT_DEADBAND_PB` (nueva constante, `config.h`, valor `20` — deriva de las ~28 cuentas ADC de ruido real medidas en banco, sesión 2026-08-13 15:10, convertidas a escala PitchBend).
  - `AUTO_OFF`/`AUTO_READ`: nunca reporta (DAW absoluto, Logic ya tiene la posición).
  - `AUTO_TRIM`/`AUTO_TOUCH`/`AUTO_LATCH`: reporta solo si `touchState==1` — letra muerta hoy (depende de `isManualTouchDetected()`, ver punto 1), gancho para cuando el capacitivo se reactive.
  - Resto (reservados/desconocidos): no reporta, igual que `_applyFaderTarget()`.
- Si aplica, se activa el bit `SLAVE_FLAG_REPORT_FADER` en `SlavePacket.buttons` y se actualiza `_rsLastReportedPB`.

**3. S2+S3 — `protocol.h` (ambas copias) — bit 6 renombrado**
- `SLAVE_FLAG_CALIB_SENDING` → `SLAVE_FLAG_REPORT_FADER`. El flag original nunca lo activaba ningún S2 (confirmado antes de reutilizar el bit) — no hay colisión con lecturas antiguas en tránsito.

**4. S3 — `main.cpp::processSlaveResponse()` + `config.h` — S3 pasa a ser transparente**
- Eliminados: `lastSentPb[9]` (estado por slave), `FADER_SYNC_DEADBAND` (200), `MOTOR_SETTLE_THRESHOLD` (60) y toda la heurística `motorSettled`/`shouldSend` que comparaba contra `faderTarget`.
- Nuevo comportamiento: si `SLAVE_FLAG_REPORT_FADER` está activo en la respuesta del S2, reenvía `faderPos` como PitchBend a Logic sin condición adicional — la decisión ya la tomó el S2.
- **Motivo del cambio:** la heurística vieja comparaba `faderPos` contra `faderTarget` para decidir "motor asentado" — en `AUTO_WRITE` el motor está inhibido (`RS485Handler::_applyFaderTarget`) y `faderTarget` queda congelado en el último valor de Logic, así que en cuanto el usuario alejaba el fader de ese valor congelado la heurística dejaba de disparar, rompiendo la escritura de automatización en WRITE.

**5. S2 — `display/Display.cpp:119` `drawCalibDot()` — círculo de estado de calibración agrandado y reposicionado**
- `tft.fillCircle(TFT_WIDTH/2, 205, 6, calibColor)` → `tft.fillCircle(TFT_WIDTH/2, 232, 12, calibColor)` (radio 6→12, y=205→232). **Motivo no registrado** — cambio presente en el working tree junto al resto de esta sesión, sin entrada previa en pendientes ni confirmación del usuario sobre la razón. Documentado aquí solo para dejar rastro del qué; si hace falta el porqué, preguntar en la próxima sesión.

**SIN VALIDAR EN BANCO** — pendiente flashear S2+S3 juntos y confirmar: READ (sin cambios), WRITE (fader movido a mano debe reportarse a Logic en todo el recorrido, no solo cerca del punto de partida), sujetar el fader parado con motor activo (debe cortar por STALL_PROTECT y no quemar el motor), y SELECT-por-touch (no tocado en esta sesión).
