...
|-----------|-------|-------|
| 🔴 Alta | **NO RESUELTO: fader S2 no converge de forma fiable al target de Logic (2026-07-20)** | Sesión larga de diagnóstico en banco S3+1×S2. Se encontraron y corrigieron varias causas reales (ver sesión 2026-07-20 abajo: `LOGIC_PITCHBEND_MAX` 14845→16383, cooldown STALL bloqueando indefinidamente, recalibración fantasma al desconectar Logic, `Motor::off()` sin resetear estado, AutoMode SysEx 0x0E sin reenviar a slave, goToMin peleando con touch del usuario). **Ninguno de estos fixes está validado en hardware de forma concluyente todavía** — la sesión terminó en limpieza de código/documentación, no en confirmación final de "funciona". Próxima sesión: repetir el test end-to-end completo (Logic conectado, READ, target lejos de la posición actual, sin tocar el fader) y confirmar converge sin STALL ni desviación. |
