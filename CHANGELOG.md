...
| 🔴 Alta | **ExPressif: JV-2080 no cambia de modo Patch↔Performance pese a SysEx correcto (2026-07-02 18:33)** | El toggle en `UIBank.cpp` (tab "SON", toque simple) cambia el label en pantalla y manda `sendSoundMode()` (SysEx DT1 "Sound Mode", `MIDIOut.cpp`), pero **el JV-2080 físico no cambia de modo**. Contenido del SysEx verificado byte a byte contra fuente externa (coincide con `F0 41 10 6A 12 00 00 00 00 <00/01> <00/7F> F7`, ver nota metodológica abajo — pendiente re-verificar contra `JV-2080_OM.pdf` p.187-188 directamente, no solo por comentario previo). Se extrajo el Device ID (byte 3) a constante nombrada `JV2080_DEVICE_ID=0x10` en `config.h` (antes hardcodeado inline) por si el panel del synth tiene un Device ID distinto de fábrica — **probado, sigue sin funcionar**, así que el Device ID no era la causa (o el usuario no llegó a probar con un valor distinto al no conocer el Device ID real del panel). Bank Select (CC0/CC32) + Program Change SÍ funcionan (cambian el programa activo), solo el SysEx de Sound Mode falla. Hipótesis pendientes: (1) el synth requiere el checksum/dirección real de "Sound Mode" que puede no coincidir con lo documentado si la fuente previa era incorrecta — releer el manual oficial línea por línea; (2) framing USB-MIDI del SysEx trockeado en `sendSysEx12()` (4 paquetes de 3 bytes, CIN 0x04/0x07) sin confirmar con MIDI Monitor en el tramo final hacia el JV-2080; (3) ruta física P4→JV-2080 (¿directo por DIN o vía Mac/Logic/interfaz?) puede estar filtrando SysEx aunque deje pasar CC/PC — sin confirmar. **Nota metodológica:** una verificación de esta sesión usó WebSearch/Sweetwater en vez del manual oficial — el usuario marcó esa fuente como no válida para este proyecto (ver `feedback_fuente_verificacion_tecnica` en memoria), aunque el dato coincidiera. Próximo paso: confirmar con MIDI Monitor qué sale realmente por USB del P4 al pulsar el toggle, y contrastar dirección/checksum contra el PDF oficial, no contra fuentes secundarias. |
| 🔴 Alta | **ExPressif: DECIDIDO — construir selector Patch/Performance/Rhythm del JV-2080 (2026-07-01 22:06)** | **Confirmado por el usuario: esto es lo que se va a implementar** (no es solo una nota de investigación). Datos MIDI **verificados** (fuente: `JV-2080_OM.pdf` p.184, leído directo del manual oficial — Bank Select CC0/CC32 + PC estándar, no SysEx). La tabla `kBanks[]`/`JVPatches.h` actual (USER/PR-A/B/C/GM/PR-E, MSB 0x50/0x51) coincide con Patch, falta CARD (MSB 0x52/82, LSB 0) y XP-A..XP-H (MSB 0x54/84, LSB 0-15 — no prioritario). **Plan de implementación (retomar 2026-07-02):** (1) tablas de banco nuevas para Performance (USER/PR-A/PR-B/CARD, PC 0-31) y Rhythm Set (mismo patrón MSB/LSB que Patch, PC 0-1); (2) `UIBANK_SON_PATCHES_BANK` deja de ser constante fija — Performance=32 patches (4 páginas), Rhythm=2 (1 página), hay que hacer el lazy-build dinámico por tipo; (3) sin nombres para Performance/Rhythm — `Roland_JV-2080_Patches_Verificado.md` solo tiene los 768 nombres de Patch, el combo mostrará solo `BANCO:NNN` sin nombre en esos dos modos; (4) **⚠️ decisión abierta pendiente de confirmar mañana:** `FavEntry` necesita campo `type` nuevo (0=PATCH 1=PERFORMANCE 2=RHYTHM) — cambia `sizeof(FavEntry)` → **rompe compatibilidad NVS con favoritos ya guardados** (`favLoad()` los trata como slot vacío si el tamaño no coincide, `FavStore.cpp:23`). Falta decidir: ¿migración automática (blob de tamaño viejo → asumir `type=PATCH`) o aceptar pérdida de favoritos actuales? (5) **ubicación del selector Patch/Performance/Rhythm sin decidir** — candidato: Tab 2 (Canal MIDI), como tercera fila de control. |
