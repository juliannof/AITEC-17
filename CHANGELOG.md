...
| 🔴 Alta | **P4: arranque sin monitor — CDC bloquea setup() (2026-06-11)** | `main.cpp` line 207: `Serial.begin(115200)` sin `Serial.setTxTimeoutMs(0)` a continuación. Con `ARDUINO_USB_CDC_ON_BOOT=1`, el core escribe al CDC antes de setup() y bloquea hasta que un host abre el puerto → pantalla negra hasta abrir monitor serie. **Fix:** añadir `Serial.setTxTimeoutMs(0);` inmediatamente después de `Serial.begin(115200)` en `setup()`. Confirmado en sesión 2026-06-09: el fix se discutió pero no se commitió. |
| 🟡 Media | **P4: Touch GT911 inoperativo — `i2c transaction failed` en boot (2026-06-11)** | `Display.cpp`: `esp_lcd_touch_new_i2c_gt911()` retorna error → `s_tp=NULL`, indev no registrado, touch completamente deshabilitado. Workaround (guard) aplicado para evitar boot loop. Causa pendiente confirmar: posiblemente pull-ups internos I2C no habilitados (`flags.enable_internal_pullup`), o conflicto de bus. Demo fabricante (`JC1060P470C_I_W-main`) usa `I2C_NUM_0` + pull-ups; nuestro driver usa `I2C_NUM_1`. Verificar con `i2c_master_probe()` antes de `esp_lcd_touch_new_i2c_gt911`. |
| 🟢 Baja | **P4: CPU 400MHz → 360MHz en platformio.ini (2026-06-11)** | `platformio.ini`: `board_build.f_cpu = 400000000L`. ESP32-P4 ES (Engineering Sample) soporta máximo 360MHz → warning en build. **Fix:** cambiar a `360000000L`. Sin impacto funcional observable. |
| 🟢 Baja | **P4: eliminar constantes portrait legado de config.h (2026-06-11)** | `config.h` líneas 70-73: `MENU_PANEL_H`, `HEADER_X`, `HEADER_W` marcadas con `// TODO landscape`. Ninguna página UI las usa ya (UIMenu/Page1/Page3/Page3B migradas a landscape nativo). **Fix:** eliminar los 3 defines y el comentario TODO. Validar con grep antes de borrar. |
