...
| 🔴 Alta | **P4: fix de guard de familia en handshake MCU (`processMackieSysEx`), SIN VALIDAR EN BANCO (2026-08-13 16:48)** | El P4 respondía a sondeos de CUALQUIER familia Mackie (0x10/0x11/0x15/0x17), no solo la propia (0x14) — confirmado con MIDI Monitor real, bug ya documentado en `docs/MIDI.md` §3.4.1 desde 2026-05-24 pero nunca aplicado. Guard `device_family != DEVICE_FAMILY` movido al inicio de la función. Pendiente reflashear P4 y confirmar en MIDI Monitor que la secuencia de conexión deja de repetirse en bucle. Ver sesión 2026-08-13 16:48 abajo. |
| 🟡 Media | **S3: mismo bug de familia hardcodeada que P4, deliberadamente SIN TOCAR (2026-08-13 16:48)** | El S3 tiene el mismo patrón de código (guard de familia después de la Fase 0 del handshake) — pero el usuario confirmó que el S3 conecta bien en Logic tal como está, así que no se aplica el fix para no arriesgar un comportamiento ya validado en banco. Si aparecen síntomas de loop de handshake en S3 en el futuro, revisar `midi/MIDIProcessor.cpp::processMackieSysEx()` primero. |
...
| 🔴 **CRÍTICA — CONFIRMADO GRAVE, EXTENDER INUSABLE CON P4+S3 SIMULTÁNEOS** | **S3: `DEVICE_FAMILY=0x14` (idéntico al P4) rompe el reconocimiento MCU de Logic cuando ambos MCU están conectados a la vez (2026-08-16 22:43, actualizado)** | Ver sesión 2026-08-16 abajo — cambio original necesario porque Logic nunca manda las notas de AutoMode (74-78) a un dispositivo identificado como Extender, solo al Main Unit, confirmado en banco. **Riesgo ya documentado antes de probar en banco con los dos MCU juntos: confirmado.** El usuario reporta, con P4 y S3 conectados simultáneamente a Logic: **"en algún momento, Logic decide que no son MCU y deja de responder correctamente"** — la extender queda inusable en ese estado. Sin captura de MIDI Monitor todavía que precise el instante/trigger exacto (¿al reconectar? ¿tras un bank switch? ¿tras el ciclo periódico de keep-alive `0x21` cada ~1s? ver `docs/MIDI.md` §3.3/§4.2). **Hipótesis de trabajo (sin confirmar):** el gestor de superficies de control de Logic probablemente no espera dos dispositivos respondiendo con la misma identidad de familia (`0x14`) simultáneamente — al no poder diferenciar cuál es cuál en su topología interna (1 Main Unit + N Extenders), puede acabar invalidando el reconocimiento MCU de ambos o de uno de los dos. **Decisión del usuario (2026-08-16 22:43): NO revertir por ahora — solo documentar.** `DEVICE_FAMILY`/`VERSION_REPLY_CMD` se mantienen en `0x14` en `S3/config.h:17-18` (fallback documentado a `0x15` sigue disponible ahí si se decide revertir más adelante, aunque eso reintroduce el problema original de que Logic no manda AutoMode al Extender). **Estado real: el sistema con P4+S3 simultáneos está roto — pendiente de decidir estrategia** (¿revertir y perder AutoMode en S3? ¿investigar si existe una forma de que Logic acepte dos Main Unit — quizás vía SysEx `0x0C` tipo de superficie, o algún otro campo de diferenciación en el handshake? ¿separar el AutoMode del mecanismo de identidad de familia por completo?). Dato previo sin resolver, sigue pendiente: AutoMode "funciona OK hasta la pista 3/4" en pruebas anteriores con family=0x14, causa no diagnosticada (ver histórico abajo). |
