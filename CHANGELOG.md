...
| 🔴 **CRÍTICA** | **S3: `DEVICE_FAMILY` cambiado a `0x14` (idéntico al P4) como fix del AutoMode, RIESGOS DE BANKING/HANDSHAKE SIN VALIDAR + AutoMode falla a partir de cierta pista, SIN DIAGNOSTICAR (2026-08-16)** | Ver sesión 2026-08-16 abajo — cambio necesario porque Logic nunca manda las notas de AutoMode (74-78) a un dispositivo identificado como Extender, solo al Main Unit, confirmado en banco. Pero este mismo valor (`0x14` en la rama `DEVICE_S3_EXTENDER`) ya causó antes, por error de copia, que "Logic no pudiera distinguir master de extender" (fix `2026-08-07`, comentario en `S3/config.h:12-16`) — no se conoce el síntoma exacto de aquella vez. **Falta confirmar explícitamente:** (1) que el banking de canales se mantiene sincronizado entre P4 y S3 al usar Bank Left/Right en cualquiera de los dos; (2) que la conexión general del S3 sigue estable sin loops de handshake ni desconexiones. Si cualquiera falla, revertir `DEVICE_FAMILY`/`VERSION_REPLY_CMD` de `0x14` a `0x15` en `S3/config.h:17-18` — el comentario en el propio archivo documenta el revert. **Además, el usuario reportó al cierre de la sesión que el AutoMode "funciona OK hasta la pista 3" (luego corregido a "4 en realidad", sin aclarar si el corte es en la pista 4 o a partir de la 5) — dato sin confirmar ni diagnosticar.** No se determinó si el corte está en el cálculo del propio S3 (revisar `[AUTOMODE-ALL]`), en el transporte RS485 (los `ID MISMATCH` ya conocidos, fila arriba, podrían ser la causa — corrompen paquetes a partir de cierto slave), o en el render del S2. Primer paso al retomar: pedir la línea `[AUTOMODE-ALL] ch0-7: ...` de S3 en el momento del fallo junto con lo que se ve físicamente en la pista que falla. |
