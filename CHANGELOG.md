...
|-----------|-------|-------|
| 🔴 Alta | **S2: separar PWM de arranque (kick) del PWM de posicionamiento fino — brief completo en sesión 2026-08-13 14:17 abajo** | Un solo par `pwmMin`/`pwmMax` por unidad (NVS) sirve tanto para vencer fricción estática al arrancar desde parado como para el movimiento fino cerca del target — con `pwmMin=125` (más alto que el default de fábrica 100), el "suelo" de velocidad cerca del target sigue siendo agresivo pese a dos rondas de ajuste de rampa de frenado en la sesión de hoy. Requiere sesión dedicada con iteración en banco (no delegable a un agente sin acceso al hardware). Ver brief de diseño al final de la sesión 2026-08-13. |
