...
Captura de MIDI Monitor durante:
1. **Playback con señal activa** — ¿cuántos mensajes por segundo? ¿Logic manda el mismo valor repetido o solo en cambios?
2. **Silencio entre notas** — ¿Logic manda 0 explícitamente o simplemente para de mandar?
3. **Stop de playback** — ¿Logic manda 0 para todos los canales al parar?
4. **Canales sin pista activa** — ¿Logic manda 0 continuamente o no manda nada?

Capturar: timestamp, status byte, data byte (strip+level codificados).
